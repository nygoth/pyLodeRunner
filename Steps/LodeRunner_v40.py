...
# Очередной этап завершён. У нас плавно двигается персонаж с учётом элементов уровня, падает, собирает сокровища
# и получает лестницу на выход

# Задача текущего этапа -- анимация. Суть её проста -- спрайт представлен набором картинок, которые выводятся
# последовательно через определённый интервал времени. Для персонажей сложнее. Там анимация зависит от направления
# движения, спрайта под ними и состояния (падение, стояние, висение, заперт в ловушке, умирает, съедают)
...
TEMPO = 18  # Количество ключевых кадров в секунду. Темп игры
FPS = TEMPO * STEP
...
PLAYER_FRAMES = {"idle": ("character_maleAdventurer_idle.png",),
                 "fall": ("character_maleAdventurer_fall.png",),
                 "hang": ("character_maleAdventurer_hang.png",),
...
BEAST_FRAMES = {"idle": ("character_zombie_idle.png",),
                "fall": ("character_zombie_fall.png",),
            ...
            for state in STATES.keys():
                if state in img:
                    ...
                    for file in img[state]:
                        self.images[STATES[state]].append(Block(os.path.join(subfolder, file)))
    ...
    def get_image(self, tick):
        if self.move_direction * self.move_state in self.images:
            if isinstance(self.images[self.move_direction * self.move_state], list):
                ...
                current_frame = int(tick / ((STEP + 1) / frames))
